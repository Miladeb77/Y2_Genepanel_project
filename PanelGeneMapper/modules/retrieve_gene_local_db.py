...
def retrieve_latest_panelapp_db(archive_folder=None, panelapp_db=None):
    ...
    try:
        ...
        # Use the specified PanelApp database if provided
        ...
        if archive_folder:
            archived_files = [
                f for f in os.listdir(archive_folder) if f.startswith("panelapp_v") and f.endswith(".db.gz")
            ]
            if archived_files:
                ...
                temp_file = os.path.join("/tmp", latest_archived.replace(".gz", ""))
...
def connect_and_join_databases(
    ...
    """
    try:
        # Ensure database and archive directories
        databases_dir = get_databases_dir()
        archive_folder = archive_folder or get_archive_dir()
        ...
        # Retrieve the latest PanelApp database if not provided
        panelapp_path, is_temp = retrieve_latest_panelapp_db(archive_folder, panelapp_db)
        ...
        for date in unique_dates:
            ...
            panelapp_path, is_temp = retrieve_latest_panelapp_db(
                archive_folder, os.path.join(databases_dir, panelapp_file)
            )
