...
def test_fetch_multiple_records_with_r133(test_client):
    """
    Test the /rcode endpoint to ensure it retrieves 
    multiple patient records associated with R Code 'R133', which will NOT be present in the
    most recent panelapp database but present in one of the older PanelApp databases.

    Asserts
    -------
    - The HTTP response status code is 200.
    - The response contains all patient records associated with 'R133'.
    """
    # Arrange: Create an older PanelApp database with relevant data for 'R133'
    older_panel_db_path = os.path.join(test_client.application.config['PANEL_DIR'], "panelapp_v20220101.db.gz")

    # Create the older PanelApp database with 'R133' and multiple gene panel records
    with gzip.open(older_panel_db_path, 'wb') as f_out:
        with tempfile.NamedTemporaryFile(delete=False) as temp_db_file:
            ...
            """)
            # Insert multiple entries for 'R133' into the older database
            cursor.execute("""
                INSERT INTO panel_info (gene_symbol, hgnc_id, relevant_disorders, version_created)
                VALUES ('GeneX', 'HGNC:98765', 'R133', '2022-01-01')
            """)
            cursor.execute("""
                INSERT INTO panel_info (gene_symbol, hgnc_id, relevant_disorders, version_created)
                VALUES ('GeneY', 'HGNC:56789', 'R133', '2022-01-01')
            """)
            cursor.execute("""
                INSERT INTO panel_info (gene_symbol, hgnc_id, relevant_disorders, version_created)
                VALUES ('GeneZ', 'HGNC:13579', 'R133', '2022-01-01')
            """)
            conn.commit()
            conn.close()
    ...
    # Arrange: Insert multiple patient records with 'R133' into the patient database
    conn = sqlite3.connect(test_client.application.config['PATIENT_DB_PATH'])
    cursor = conn.cursor()
    # Insert first patient record
    cursor.execute("""
        INSERT INTO patient_data (patient_id, clinical_id, test_date, panel_retrieved_date)
        VALUES (?, ?, ?, ?)
    """, ("Patient_11111", "R133", "2024-12-25", "2022-01-01"))
    # Insert second patient record
    cursor.execute("""
        INSERT INTO patient_data (patient_id, clinical_id, test_date, panel_retrieved_date)
        VALUES (?, ?, ?, ?)
    """, ("Patient_22222", "R133", "2024-12-26", "2022-01-01"))
    # Insert third patient record
    cursor.execute("""
        INSERT INTO patient_data (patient_id, clinical_id, test_date, panel_retrieved_date)
        VALUES (?, ?, ?, ?)
    """, ("Patient_33333", "R133", "2024-12-27", "2022-01-01"))
    conn.commit()
    conn.close()

    # Act: Send a GET request to fetch records for 'R133'
    response = test_client.get('/rcode?r_code=R133')
    ...
    # Verify each patient record
    expected_patient_ids = {"Patient_11111", "Patient_22222", "Patient_33333"}
    actual_patient_ids = {record['patient_id'] for record in data}
    assert actual_patient_ids == expected_patient_ids, "Patient IDs do not match expected values."

    # Optionally, verify other fields if necessary
    for record in data:
        assert record['relevant_disorders'] == "R133", "Relevant disorder does not match 'R133'."
        assert 'gene_panel' in record, "gene_panel field is missing in the record."
        assert 'hgnc_ids' in record, "hgnc_ids field is missing in the record."
